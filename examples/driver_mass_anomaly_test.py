...
def integrate_mass_and_barycentre(x, y, z, rho, dx_cell=2.0):
    ...
    if M <= 0:
        raise ValueError("Total mass is non-positive. Check rho values/units.")
...
def mean_density_sections(x, y, z, rho):
    """
    Lab statement:
      xz plane -> mean along axis=0
      yz plane -> mean along axis=1
      xy plane -> mean along axis=2
    """
    rho_xz = rho.mean(axis=0)
    rho_yz = rho.mean(axis=1)
...
#Finding slice of non-zero density.
def pick_non_negligible_region(x, y, z, rho, frac=0.10):
    threshold = frac * float(np.max(rho))
    mask = rho >= threshold
    if not np.any(mask):
        raise ValueError("No cells above threshold, lower fraction.")
    ...
    return threshold, mean_region, xr, yr, zr, mask


def forward_model_density(
    x, y, z, rho, x_s, y_s, z_obs, dx_cell=2.0, rho_frac_cut=0.10
):
    dV = dx_cell**3
    ...
    #Approximating each cell as a point mass at its center:
    rho_cut = rho_frac_cut * float(rho_flat.max())
    keep = rho_flat >= rho_cut

    m_cells = rho_flat[keep] * dV
    r_cells = np.column_stack([x_flat[keep], y_flat[keep], z_flat[keep]])
...
def second_derivative_x(F, dx):
    d2 = np.full_like(F, np.nan, dtype=float)
...
def main():
    ...
    # ---- 7.2 Mean density cross-sections ----
    (X_xz, Z_xz, R_xz), (Y_yz, Z_yz, R_yz), (X_xy, Y_xy, R_xy) = mean_density_sections(x, y, z, rho)

    vmin = float(min(R_xz.min(), R_yz.min(), R_xy.min()))
    vmax = float(max(R_xz.max(), R_yz.max(), R_xy.max()))
    ...
    out1 = outputs / "mean_density_sections.png"
    fig.savefig(out1, dpi=300)
    plt.close(fig)
    print(f"Saved: {out1}")

    # ---- 7.3 Non-negligible region ----
    threshold, mean_region, xr, yr, zr, _mask = pick_non_negligible_region(x, y, z, rho, frac=0.10)

    print("\n=== Non-negligible Region (rho >= 10% max) ===")
    print(f"Threshold used: {threshold:.6e} kg/m^3")
    print(f"x range: {xr}")
    print(f"y range: {yr}")
    print(f"z range: {zr}")
    print(f"Mean density in region: {mean_region:.6e} kg/m^3")

    # ---- 7.4 / 7.5 Forward modelling at z=0,1,100,110 (dx=5m) ----
    dx_s = 5.0
    pad = 10.0
    x_s = np.arange(float(x.min()) - pad, float(x.max()) + pad + dx_s, dx_s)
    y_s = np.arange(float(y.min()) - pad, float(y.max()) + pad + dx_s, dx_s)
    ...
    for ax, zz in zip(axes, z_levels):
        ...
        ax.set_title(f"gz at z={zz} m (dx=5 m)")
        ax.set_xlabel("x (m)")
        ax.set_ylabel("y (m)")

    # One shared colorbar for all panels
    fig.colorbar(last_cf, ax=axes, shrink=0.9)

    out2 = outputs / "gz_4_elevations.png"
    fig.savefig(out2, dpi=300)
    plt.close(fig)
    print(f"Saved: {out2}")

    # ---- 7.7 d2gz/dz2 via Laplace ----
    d2z0 = -(second_derivative_x(gz_maps[0.0], dx_s) + second_derivative_y(gz_maps[0.0], dx_s))
    d2z100 = -(second_derivative_x(gz_maps[100.0], dx_s) + second_derivative_y(gz_maps[100.0], dx_s))
    ...
    cL = axes[0].contourf(X, Y, d2z0, 30, vmin=vmin2, vmax=vmax2, cmap="viridis")
    axes[0].set_title(r"$\partial^2 g_z/\partial z^2$ at z=0 m")
    axes[0].set_xlabel("x (m)")
    axes[0].set_ylabel("y (m)")
    fig.colorbar(cL, ax=axes[0])

    cR = axes[1].contourf(X, Y, d2z100, 30, vmin=vmin2, vmax=vmax2, cmap="viridis")
    axes[1].set_title(r"$\partial^2 g_z/\partial z^2$ at z=100 m")
    axes[1].set_xlabel("x (m)")
    axes[1].set_ylabel("y (m)")
    fig.colorbar(cR, ax=axes[1])

    out3 = outputs / "d2gz_dz2_z0_z100.png"
